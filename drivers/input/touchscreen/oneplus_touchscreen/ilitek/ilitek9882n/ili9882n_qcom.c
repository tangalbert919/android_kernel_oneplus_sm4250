...
void ili_report_gesture_mode(u8 *buf, int len)
{
	...
	case GESTURE_E:
		...
		break;
	case GESTURE_S:
		gc->type  = Sgestrue;/*ritchie add for gesture s*/
		gc->clockwise = 1;
		break;
	case GESTURE_Z:
		gc->type  = GESTURE_Z;
...
int ili_set_tp_data_len(int format, bool send, u8* data)
{
	...
	case DATA_FORMAT_DEBUG_LITE_AREA:
		/*if(cmd == NULL) {
			...
			return -1;
		}*/ /*ritchie add for modify 20200824*/
...
#else
	if (ilits->ts->boot_mode == MSM_BOOT_MODE_RECOVERY
		|| is_oem_unlocked() || ilits->ts->fw_update_in_probe_with_headfile) {
		ret=ili_fw_upgrade_handler();
		if (ret < 0) {
			ILI_ERR("FW upgrade failed during moving code\n");
...
#else
	    if ((ilits->ts->boot_mode == MSM_BOOT_MODE_FACTORY ||
		ilits->ts->boot_mode == MSM_BOOT_MODE_RF))
#endif
	    {
            ...
            ili_ic_func_ctrl("sleep", SLEEP_IN_FTM_END);
            mutex_unlock(&ilits->touch_mutex);
            msleep(60);
...
static int ilitek_mode_switch(void *chip_data, work_mode mode, bool flag)
{
    ...
    case MODE_EDGE:
        ILI_INFO("g_tp->limit_switch = %d\n", g_tp->limit_switch);
        if (g_tp->limit_switch == 0) {
            temp[0] = 0x01;
        } else if (g_tp->limit_switch == 1) {
            temp[0] = 0x02;
        } else if (g_tp->limit_switch == 2) {
            temp[0] = 0x07;
                                        ...
                                        bool force)
{
    ...
    chip_info->actual_tp_mode = P5_X_FW_AP_MODE;
	ilits->oneplus_fw_update = true;
    ret = ili_fw_upgrade_handler();
	if (ret < 0) {
		ILI_ERR("FW upgrade failed during moving code\n");
	}
	ilits->oneplus_fw_update = false;
    mutex_unlock(&chip_info->touch_mutex);
    if (ret < 0) {
        ILI_ERR("Failed to upgrade firmware, ret = %d\n", ret);
        return ret;
...
static bool ilitek_irq_throw_away(void *chip_data)
{
    struct ilitek_ts_data *chip_info = (struct ilitek_ts_data *)chip_data;
	ILI_DBG("\n");
    if ((ilits->actual_tp_mode == P5_X_FW_TEST_MODE) || atomic_read(&chip_info->cmd_int_check) == ENABLE) {
...
static struct oneplus_touchpanel_operations ilitek_ops = {
                                  ...
                                  u8 read_type)
{
    ...
    enable_irq(ilits->irq_num);//because oneplus disable
...
static int ilitek_create_proc_for_oneplus(struct touchpanel_data *ts)
{
    int ret = 0;
...
static void ili_init_oneplus_apk_op(struct touchpanel_data *ts)
{
...
int __maybe_unused ilitek9882n_spi_probe(struct spi_device *spi)
{
	...
	//ili_irq_enable();
    ilitek_create_proc_for_oneplus(ts);
#ifdef CONFIG_ONEPLUS_TP_APK
	ilits->demo_debug_info[0] = ili_demo_debug_info_id0;
	ili_init_oneplus_apk_op(ts);
#endif // end of CONFIG_ONEPLUS_TP_APK
