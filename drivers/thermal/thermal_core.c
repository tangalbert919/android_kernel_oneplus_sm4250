...
/* GuangYan.hong@BSP.POWER.Secure 2020/04/09, Add for print skin_therm temp  begin*/
#define PRINT_SKIN_THERM_TEMP_BY_TIMES    10000
struct delayed_work temp_work;
/* GuangYan.hong@BSP.POWER.Secure 2020/04/09, Add for print skin_therm temp  end*/
...
static int update_thermal_target(struct thermal_zone_device *tz, unsigned long val, int thermal_type)
{
	...
	thermal_zone_device_update(tz, THERMAL_EVENT_UNSPECIFIED);
	ret = tz->ops->set_trip_temp(tz, (trip+1), (temperature + 2)*1000);
	if (ret)
		return ret;
	thermal_zone_device_update(tz, THERMAL_EVENT_UNSPECIFIED);
	if ((temperature + 2) > 51) {
		ret = tz->ops->set_trip_temp(tz, (trip+2), (temperature + 3)*1000);
		if (ret)
			return ret;
		ret = tz->ops->set_trip_temp(tz, (trip+3), (temperature + 4)*1000);
		if (ret)
			return ret;
		thermal_zone_device_update(tz, THERMAL_EVENT_UNSPECIFIED);
		adjust = 1;
	} else if (adjust) {
		ret = tz->ops->set_trip_temp(tz, (trip+2), (temperature + 4)*1000);
		if (ret)
			return ret;
		ret = tz->ops->set_trip_temp(tz, (trip+3), (temperature + 6)*1000);
		if (ret)
			return ret;
		thermal_zone_device_update(tz, THERMAL_EVENT_UNSPECIFIED);
		adjust = 0;
...
/* GuangYan.hong@BSP.POWER 2020/04/09, Add for print skin_therm temp begin */
static void get_skin_therm_work(struct work_struct *work)
{
	struct thermal_zone_device *skin_therm;
	int ret, temp = 0;

	char name[20] = "skin-therm";
...
/* GuangYan.hong@BSP.POWER 2020/04/09, Add for print skin_therm temp end */


int get_charging_therm_temp(void)
{
        struct thermal_zone_device *skin_therm;
        int ret, charging_temp = 0;

        char name[20] = "skin-therm";

        skin_therm = thermal_zone_get_zone_by_name(name);
        ret = thermal_zone_get_temp(skin_therm, &charging_temp);

        if (!IS_ERR(skin_therm))
                pr_err("%s: tz=%s, oneplus charging_temp=%d\n", __func__, skin_therm->type, charging_temp);

        return charging_temp;
}
EXPORT_SYMBOL(get_charging_therm_temp);
