...
block_t f2fs_get_unusable_blocks(struct f2fs_sb_info *sbi)
{
	...
	if (f2fs_dis_cp_ratio) {
		f2fs_info(sbi, "Debug:%s:disable cp log_blocks_per_seg %u thresh %u holes_data %u holes_node %u"
				, __func__, (sbi->log_blocks_per_seg), thresh, holes[DATA], holes[NODE]);
		thresh = (sbi->user_block_count >> 10) * f2fs_dis_cp_ratio;
		if (unusable > thresh)
			return unusable - thresh;
		return 0;
	}
