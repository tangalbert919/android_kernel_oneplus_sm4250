...
#define D(x...) pr_debug("[PA][AW87359] " x)
#define I(x...) pr_info("[PA][AW87359] " x)
#define E(x...) pr_err("[PA][AW87359] " x)
/*****************************************************************
* aw87359 marco
******************************************************************/
...
int aw87359_read_chipid(struct aw87359 *aw87359)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		...
		if (reg_val == AW87359_CHIPID) {
			I("%s: This Chip is aw87359 chipid = 0x%x\n",
					__func__, reg_val);
			return 0;
		}
		cnt++;
...
aw87359_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		E("%s: check_functionality failed\n", __func__);
		ret = -ENODEV;
	...
	if (ret < 0) {
		E("%s: aw87359_read_chipid failed ret = %d\n",
			__func__, ret);
	...
	/* aw87359 soft reset */
	aw87359_i2c_write(aw87359, 0x00, 0xaa);
...
static void aw87359_i2c_shutdown(struct i2c_client *client)
{
	I("%s\n", __func__);
	aw87359_audio_off();
}
...
EXPORT_SYMBOL(aw87359_audio_dspk);
EXPORT_SYMBOL(aw87359_audio_drcv);
EXPORT_SYMBOL(aw87359_audio_off);
